...
import RazorpayUPIPayment from "../components/RazorpayUPIPayment";
...
export default function PlanSelectionPage() {
  ...
  const planDetails = {
    BASIC: { desc: t('feat1Desc'), features: [t('feat1Title'), t('feat2Title')] },
    STANDARD: { desc: t('feat2Desc'), features: [t('feat1Title'), t('feat2Title'), t('feat4Title')] },
    PREMIUM: { desc: t('feat3Desc'), features: [t('feat1Title'), t('feat2Title'), t('feat3Title'), t('feat4Title')] },
  };
  ...
  const handlePaymentSuccess = ({ paymentId, policy }) => {
    setMessage(`✅ Payment verified! Policy #${policy.id} activated. Redirecting...`);
    ...
    <div className="mx-auto max-w-6xl space-y-10">
      ...
      <section className="rounded-[40px] border border-white/10 bg-white/5 p-8 backdrop-blur-xl shadow-2xl md:p-12">
        <div className="flex items-center gap-4 mb-6">
          <Logo size={48} />
          <h2 className="text-4xl font-black tracking-tight text-white md:text-5xl">{t('selectProtection')}</h2>
        </div>
      ...
      <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr]">
        ...
        <section className="rounded-[40px] border border-white/10 bg-white/5 p-8 backdrop-blur-xl shadow-2xl md:p-10">
          <div className="mb-10 pb-8 border-b border-white/5">
            <h3 className="text-2xl font-black text-white">{t('planConfig')}</h3>
            <p className="mt-1 text-sm text-white/40">{t('planConfigDesc')}</p>
          </div>

          <div className="space-y-8">
            <div>
              <label className={labelClass}>{t('chooseTier')}</label>
              <div className="grid gap-3 sm:grid-cols-3">
                {plans.map((p) => (
                  <button
                    key={p}
                    onClick={() => { setForm({ planType: p }); setQuote(null); setMessage(''); }}
                    className={`rounded-2xl border px-4 py-4 text-sm font-black uppercase tracking-widest transition duration-300 ${form.planType === p
            ...
            >
              {loading ? t('calculating') : t('calcQuote')}
            </button>
          </div>
        </section>

        {/* QUOTE + PAYMENT COLUMN */}
        <section className="rounded-[40px] border border-indigo-500/10 bg-indigo-500/5 p-8 backdrop-blur-xl shadow-2xl md:p-10 flex flex-col items-center justify-center">
          <div className="w-full text-center mb-8">
            <h3 className="text-2xl font-black text-white">{t('dynamicQuote')}</h3>
            <p className="text-sm text-indigo-300/40 mt-1 uppercase tracking-widest font-bold">{t('poweredByMl')}</p>
          </div>
          ...
          {quote ? (
            <div className="w-full space-y-6">
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="rounded-[28px] bg-white/5 border border-white/10 p-6 text-center">
                  <p className="text-[10px] font-black uppercase tracking-widest text-white/30">{t('weeklyPremium')}</p>
                  <p className="mt-2 text-4xl font-black text-white">₹{quote.weeklyPremium}</p>
                </div>
                <div className="rounded-[28px] bg-white/5 border border-white/10 p-6 text-center">
                  <p className="text-[10px] font-black uppercase tracking-widest text-white/30">{t('weeklyCap')}</p>
              ...
              <RazorpayUPIPayment
                amountPaise={Math.round(quote.weeklyPremium * 100)}
                planType={form.planType}
                user={user}
            ...
            <div className="text-center py-12">
              ...
              <p className="text-white/40 text-sm font-bold leading-relaxed max-w-[220px] mx-auto italic uppercase tracking-widest">
                {t('awaitingCalc')} {form.planType} {t('tier')}
              </p>
            </div>
          )}
